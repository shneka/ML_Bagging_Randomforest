function [error_train_ans,error_test_ans] = random_forest(training_set,test_set,column,N)

% where M is the number of base classes 
M = 100;
...
for no = 1:M
    ...
    for feat_count=1:2
        if feat_count == 1  
%             sample_set = randsample(c_train-1,N);
%             f_1 = 1;
%             for m_1 = 1:N
%                 if f_1 == 1
%                     x_new = x_train(:,sample_set(m_1));
%                     f_1 = 0;
%                 else
%                     x_new = horzcat(x_new,x_train(:,sample(m_1)));
%                 end
%             end
        
            [feature_1,split_1] = feature_select(x_train,y_train,c_train-1,final_feature(no,1),N);
%             [~,ind_1] = max(feature_1(1,:));
             final_feature(no,1) = feature_1;
             final_split(no,1) = split_1;
            ...
            [feature_1,split_1] = feature_select(x_train_1,y_train_1,c_train-1,final_feature(no,1),N);
%             [~,ind_1] = max(feature_1(1,:));
            final_feature(no,2) = feature_1;
            final_split(no,2) = split_1;          
            
            [feature_1,split_1] = feature_select(x_train_2,y_train_2,c_train-1,final_feature(no,1),N);
%             [~,ind_1] = max(feature_1(1,:));
            final_feature(no,3) = feature_1;
            final_split(no,3) = split_1;
